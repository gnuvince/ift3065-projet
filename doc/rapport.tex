...
\item Plus d'une centaine de tests exercent le parseur
...
TODO: Relire/réécrire!
Chaque fct correspond à une production de la grammaire.
Chaque fonction reçoit un sous-arbre de l'AST.
Chq fonction teste séquentiellement les alternatives de la production et
retourne l'AST reçu si l'une d'elles unifie l'AST.
Si aucune alternative ne correspond à l'AST, la fonction retourne faux.
Pour chaque alternative, la fonction teste tous les terminaux et non-terminaux
de la production et retourne vrai s'ils sont tous respectés.

Nous avons choisi cette forme déclarative pour le parseur car elle permet
d'exprimer une structure de code calquée sur les productions de la
grammaire.
